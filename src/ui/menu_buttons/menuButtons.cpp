...
void MenuButtons::loadMenuButtons(wxWindow* root_received)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        if (i == 0)
        {
            menu_button[0] = new wxButton(
                root,
                wxID_ANY,
                btn_texts[0],
                wxPoint(2, 2),
                wxSize(
                    utils_local->getFontWidth(btn_texts[2]) + x_padding_menu_buttons,
                    utils_local->getFontHeight("A") + y_padding),
                wxBORDER_NONE);
        }
        else
        {
...
void MenuButtons::showWindow(int sender_id)
{
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            menu_button_window->SetPosition(wxPoint(
                menu_button[i]->GetPosition().x,
                menu_button[i]->GetPosition().y + menu_button[i]->GetSize().GetHeight()));
...
void MenuButtons::buttonDown(const wxMouseEvent& e)
{
    ...
    if (sender_id == menu_button[0]->GetId())
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            if (i == 0)
            {
                file_button_child[i] = new wxButton(
                    ...
                    wxPoint(2, 2),
                    wxSize(
                        utils_local->getFontWidth(btn_texts[0]) + x_padding,
                        utils_local->getFontHeight("A") + y_padding),
                    wxBORDER_NONE);
            }
            else
            {
                file_button_child[i] = new wxButton(
                    ...
                    btn_texts[i],
                    wxPoint(
                        2, file_button_child[i - 1]->GetPosition().y + file_button_child[i - 1]->GetSize().GetHeight()),
                    wxSize(
                        utils_local->getFontWidth(btn_texts[0]) + x_padding,
                        utils_local->getFontHeight("A") +
                            y_padding), // 10 is the lenght of the biggest text, every button's size is the same
                    wxBORDER_NONE);
            }
            ...
            file_button_child[i]->Bind(
                wxEVT_LEFT_DOWN, &ProjectManager::showDialogWindow, project_manager, file_button_child[i]->GetId());
        ...
        menu_button_window->SetSize(wxSize(
            file_button_child[0]->GetSize().GetWidth() + 4,
            buttons_horiz_sizes + 4)); // padding of 2 each side in verticaly and horizontaly
        ...
        for (int i = 0; i < 5; i++)
        {
            if (i == 0)
            {
                edit_button_child[i] = new wxButton(
                    ...
                    wxPoint(2, 2),
                    wxSize(
                        utils_local->getFontWidth(btn_texts[4]) + x_padding,
                        utils_local->getFontHeight("A") +
                            y_padding), // 7 is the lenght of the biggest text, every button's size is the same
                    wxBORDER_NONE);
            }
            else
            {
                edit_button_child[i] = new wxButton(
                    ...
                    btn_texts[i],
                    wxPoint(
                        2, edit_button_child[i - 1]->GetPosition().y + edit_button_child[i - 1]->GetSize().GetHeight()),
                    wxSize(
                        utils_local->getFontWidth(btn_texts[4]) + x_padding,
                        utils_local->getFontHeight("A") +
                            y_padding), // 7 is the lenght of the biggest text, every button's size is the same
    ...
    else if (sender_id == menu_button[2]->GetId())
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (i == 0)
            {
                window_button_child[i] = new wxButton(
                    ...
                    wxPoint(2, 2),
                    wxSize(
                        utils_local->getFontWidth(btn_texts[3]) + x_padding,
                        utils_local->getFontHeight("A") +
                            y_padding), // 7 is the lenght of the biggest text, every button's size is the same
                    wxBORDER_NONE);
            }
            else
            {
                window_button_child[i] = new wxButton(
                    ...
                    wxPoint(
                        2,
                        window_button_child[i - 1]->GetPosition().y +
                            window_button_child[i - 1]->GetSize().GetHeight()),
                    wxSize(
                        utils_local->getFontWidth(btn_texts[3]) + x_padding,
                        utils_local->getFontHeight("A") +
                            y_padding), // 7 is the lenght of the biggest text, every button's size is the same
        ...
        window_button_child[2]->Bind(
            wxEVT_LEFT_DOWN, &SettingsManager::loadSettingsDialog, settings_manager, window_button_child[2]->GetId());
        window_button_child[3]->Bind(
            wxEVT_LEFT_DOWN, &SettingsManager::loadSettingsDialog, settings_manager, window_button_child[3]->GetId());
    ...
    else if (sender_id == menu_button[3]->GetId())
    {
        ...
        help_button_child = new wxButton(
            ...
            wxPoint(2, 2),
            wxSize(
                utils_local->getFontWidth("open online manual") + x_padding,
                utils_local->getFontHeight("A") +
                    y_padding), // 6 is the lenght of the biggest text, every button's size is the same
            wxBORDER_NONE);
...
void MenuButtons::maximizeRoot(const wxMouseEvent& e)
{
    root_frame->Maximize();
}
void MenuButtons::minimizeRoot(const wxMouseEvent& e)
{
    root_frame->Iconize();
}
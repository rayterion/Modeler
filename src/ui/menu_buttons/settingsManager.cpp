...
SettingsManager::SettingsManager(
    wxWindow* root_received, wxWindow* menu_button_window_received, MenuPanel* child_window_info_ptr_received)
{
    root = root_received;
...
void SettingsManager::loadSettingsDialog(const wxMouseEvent& e)
{
    ...
    { // project settings
        ...
        settings_dialog->SetSize(
            wxSize(utils_local->getFontWidth(title_texts[0], std_font, settings_dialog) + 2 * x_padding, 10));
        ...
        for (int i = 0; i < 3; i++)
        {
            if (i == 0)
            {
                title_project[0] = new wxStaticText(
                    ...
                    title_texts[0],
                    wxPoint(
                        settings_dialog->GetSize().GetWidth() / 2 -
                            utils_local->getFontWidth(title_texts[0], std_font, settings_dialog) / 2 -
                            x_title_padding / 2,
                        20),
                    wxSize(
                        utils_local->getFontWidth(title_texts[0], std_font, settings_dialog) + x_title_padding,
                        utils_local->getFontHeight("A", std_font, settings_dialog) + 2),
                ...
                button_project[0] = new wxButton(
                    ...
                    wxSize(
                        settings_dialog->GetSize().GetWidth() - 6,
                        utils_local->getFontHeight("A", std_font, settings_dialog) + 6),
                    wxBORDER_NONE);
            }
            else
            {
                title_project[i] = new wxStaticText(
                    ...
                    wxPoint(
                        settings_dialog->GetSize().GetWidth() / 2 -
                            utils_local->getFontWidth(title_texts[0], std_font, settings_dialog) / 2 -
                            x_title_padding / 2,
                        button_project[i - 1]->GetPosition().y + button_project[i - 1]->GetSize().GetHeight() +
                            y_padding),
                    wxSize(
                        utils_local->getFontWidth(title_texts[0], std_font, settings_dialog) + x_title_padding,
                        utils_local->getFontHeight("A", std_font, settings_dialog) + 2),
            ...
            title_project[i]->SetBackgroundColour(
                wxColour(menu_window_colour - 8, menu_window_colour - 8, menu_window_colour - 8));
            title_project[i]->SetFont(std_font);
            title_project[i]->SetForegroundColour(wxColour(255, 255, 255));

            button_project[i]->SetBackgroundColour(
            ...
            button_project[i]->Bind(
                wxEVT_LEFT_DOWN, &SettingsManager::loadEnableDisableSettings, this, button_project[i]->GetId());
        }
        settings_dialog->SetSize(wxSize(
            utils_local->getFontWidth(title_texts[0], std_font, settings_dialog) + 2 * x_padding,
            button_project[2]->GetPosition().y + button_project[2]->GetSize().GetHeight() + 40));
    }
    else if (sender == 3)
    { // editor settings
        ...
        settings_dialog->SetSize(
            wxSize(utils_local->getFontWidth(title_texts[3], std_font, settings_dialog) + 2 * x_padding, 10));
        ...
        std::string title_identifier_str[] = {
            "editor_colour_theme", "lines_size", "show_grid", "show_annotations", "show_rulers"};
        ...
        for (int i = 0; i < 5; i++)
        {
            if (i == 0)
            {
                title_editor[0] = new wxStaticText(
                    ...
                    wxPoint(
                        settings_dialog->GetSize().GetWidth() / 2 -
                            utils_local->getFontWidth(title_texts[3], std_font, settings_dialog) / 2 -
                            x_title_padding / 2,
                        20),
                    wxSize(
                        utils_local->getFontWidth(title_texts[3], std_font, settings_dialog) + x_title_padding,
                        utils_local->getFontHeight("A", std_font, settings_dialog) + 2),
                    wxBORDER_NONE | wxALIGN_CENTER | wxST_NO_AUTORESIZE,
                    title_identifier_str[i]);
                ...
                title_editor[i] = new wxStaticText(
                    ...
                    wxPoint(
                        settings_dialog->GetSize().GetWidth() / 2 -
                            utils_local->getFontWidth(title_texts[3], std_font, settings_dialog) / 2 -
                            x_title_padding / 2,
                        button_editor[i - 1]->GetPosition().y + button_editor[i - 1]->GetSize().GetHeight() +
                            y_padding),
                    wxSize(
                        utils_local->getFontWidth(title_texts[3], std_font, settings_dialog) + x_title_padding,
                        utils_local->getFontHeight("A", std_font, settings_dialog) + 2),
        ...
        button_editor[2]->Bind(
            wxEVT_LEFT_DOWN, &SettingsManager::loadEnableDisableSettings, this, button_editor[2]->GetId());
        button_editor[3]->Bind(
            wxEVT_LEFT_DOWN, &SettingsManager::loadEnableDisableSettings, this, button_editor[3]->GetId());
        button_editor[4]->Bind(
            wxEVT_LEFT_DOWN, &SettingsManager::loadEnableDisableSettings, this, button_editor[4]->GetId());

        settings_dialog->SetSize(wxSize(
            utils_local->getFontWidth(title_texts[3], std_font, settings_dialog) + 2 * x_padding,
            button_editor[4]->GetPosition().y + button_editor[4]->GetSize().GetHeight() + 40));
    }
}
...
void SettingsManager::loadEnableDisableSettings(const wxMouseEvent& e)
{
    ...
    if (choice_dialog == nullptr)
    {
        choice_dialog = new wxWindow(
            root, wxID_ANY, wxPoint(parent_pos_x + parent_width, button_pos_y), wxSize(10, 10), wxBORDER_NONE);
        ...
        wxButton* disable_button = new wxButton(
            ...
            "disable",
            wxPoint(
                enable_button->GetPosition().x, enable_button->GetPosition().y + enable_button->GetSize().GetHeight()),
            wxSize(enable_button->GetSize().GetWidth(), enable_button->GetSize().GetHeight()),
            wxBORDER_NONE);

        enable_button->SetBackgroundColour(
            wxColour(menu_window_colour + 20, menu_window_colour + 20, menu_window_colour + 20));
        ...
        disable_button->SetBackgroundColour(
            wxColour(menu_window_colour + 20, menu_window_colour + 20, menu_window_colour + 20));
        ...
        choice_dialog->SetSize(wxSize(
            disable_button->GetSize().GetWidth() + 6,
            disable_button->GetPosition().y + disable_button->GetSize().GetHeight() + 3));
    }
    else
    {
        choice_dialog->Destroy();
...
void SettingsManager::loadColourSettings(const wxMouseEvent& e)
{
    ...
    if (choice_dialog == nullptr)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            colour_button[i]->SetBackgroundColour(
                wxColour(menu_window_colour + 20, menu_window_colour + 20, menu_window_colour + 20));
        ...
        choice_dialog->SetBackgroundColour(wxColour(menu_window_colour, menu_window_colour, menu_window_colour));
        choice_dialog->SetSize(wxSize(
            colour_button[0]->GetSize().GetWidth() + 6,
            colour_button[3]->GetPosition().y + colour_button[3]->GetSize().GetHeight() + 3));
    }
    else
    {
        choice_dialog->Destroy();
...
void SettingsManager::loadSizeSettings(const wxMouseEvent& e)
{
    ...
    if (choice_dialog == nullptr)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            size_button[i]->SetBackgroundColour(
                wxColour(menu_window_colour + 20, menu_window_colour + 20, menu_window_colour + 20));
        ...
        choice_dialog->SetBackgroundColour(wxColour(menu_window_colour, menu_window_colour, menu_window_colour));
        choice_dialog->SetSize(wxSize(
            size_button[0]->GetSize().GetWidth() + 6,
            size_button[2]->GetPosition().y + size_button[2]->GetSize().GetHeight() + 3));
    }
    else
    {
        choice_dialog->Destroy();
